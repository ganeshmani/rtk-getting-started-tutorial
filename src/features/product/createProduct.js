...
import { createProductMutation, createProductApiStateSelector } from "./productSlice";
const CreateProduct = ({ isOpen, handleModalClick }) => {
  const dispatch = useDispatch();

  const createProductApiState = useSelector(createProductApiStateSelector);
  ...
  return (
    <Modal isOpen={isOpen} setOpen={handleModalClick}>
      <form onSubmit={_handleCreateProduct}>
        <div>
          <h4 className="mt-1 text-center text-3xl font-extrabold text-gray-900">
            Create Product
          </h4>
          <div>
            <label
